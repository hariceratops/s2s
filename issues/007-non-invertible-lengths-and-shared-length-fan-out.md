...
- The criterion "identifying the length field **and** the disagreeing
  ...
  disagreeing dependent is still not possible as specified. **Deferred
  2026-08-07** to the "Error ergonomics" backlog item in `doc/todos.md`, which
  carries the design context and shares a root cause with issue 006. `cast_error` carries
...
- "Overflow checking from 004 applies to the computed length as well" is met
  ...
  to overflow.
- **Closed 2026-08-07.** Design §4.2 asked for this criterion to be added
  here and it was not. A `len_from_field` target whose dependent lives inside
  a `maybe` is a *conditional* source: it stays writable, and its stored value
  is now verified against the producer whenever the producer is actually
  present (`verify_conditional_len`). Covered in
