...
runs it, because the one example this repo had sat unbuilt in `test/hello/`
and went stale unnoticed. An example must return non-zero when it fails, and
any input file it reads is generated by a CTest fixture rather than assumed to
exist.
