...
TEST(UnionWrite, DerivesTheDiscriminantFromTheHeldAlternative) {
...
TEST(UnionWrite, RoundTripsSwitchUnionsInBothByteOrders) {
  using test_field_list = switch_schema;
...
TEST(UnionWrite, RecursesIntoRecordAlternatives) {
  using test_field_list = switch_schema;
...
TEST(UnionWrite, RoundTripsLadderUnionsWhenTheHeldAlternativeAgrees) {
...
TEST(UnionWrite, RejectsALadderAlternativeThePredicatesWouldNotSelect) {
  using test_field_list = ladder_schema;
...
TEST(UnionWrite, RejectsAComputedSwitchAlternativeThatDisagrees) {
...
TEST(UnionWrite, RoundTripsComputedSwitchUnionsWhenTheyAgree) {
  using test_field_list = computed_schema;
...
TEST(UnionWrite, PropagatesTypeDeductionFailure) {
...
TEST(UnionWrite, FieldsFeedingLadderPredicatesStayAssignable) {
  ladder_schema obj{};
  obj["a"_f] = 3;
  obj["b"_f] = 4;
