...
You get this by using the library. It holds with a plain `std::ifstream`, with a
schema full of `std::string` and `std::vector`, on gcc 13, in a debug build.
Nothing below is required for it.
...
than raw pointers. See [Constexpr streams](#constexpr-streams). A
`std::ifstream` cannot be one — there are no files during constant evaluation.

**The schema must have no allocating fields.** Anything that would need the heap
is disqualifying:
...
value you can `static_assert` on.
